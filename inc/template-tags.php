...
if ( ! function_exists( 'big_brother_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function big_brother_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'big-brother' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'big-brother' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					<?php if ( 0 != $args['avatar_size'] ) echo get_avatar( $comment, $args['avatar_size=24&type=comment'] ); ?>
					<?php printf( __( '%s', 'big-brother' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->

				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'big-brother' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
					<?php edit_comment_link( __( 'Edit', 'big-brother' ), '<span class="edit-link">', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'big-brother' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
...
endif; // ends check for big_brother_comment()

if ( ! function_exists( 'big_brother_the_attached_image' ) ) :
/**
 * Prints the attached image with a link to the next attached image.
 */
function big_brother_the_attached_image() {
	$post                = get_post();
	$attachment_size     = apply_filters( 'big_brother_attachment_size', array( 1200, 1200 ) );
...
function big_brother_posted_on() {
	...
	printf( __( '<span class="posted-on">%1$s</span>', 'big-brother' ),
